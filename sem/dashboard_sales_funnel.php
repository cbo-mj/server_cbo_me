...
$target_number = $_GET["id"];
	...
	$sql_year_graph =  "SELECT sum(c.impressions) as impressions, sum(c.clicks) as clicks, sum(a.`event`) as event, sum(d.calls) as calls from ((SELECT a.date,a.adwordsCampaignID, sum(a.`totalEvents`) as `event` FROM `ga_event_category_history_data` a GROUP BY a.date,a.adwordsCampaignID) a inner join group_campaign_link b on a.adwordsCampaignID = b.campaign_id inner join (SELECT c.date, c.adwordsCampaignID, sum(c.impressions) as impressions, sum(c.adClicks) as clicks from ga_adword_campaign_data_history c group by c.date, c.adwordsCampaignID) c on a.adwordsCampaignID = c.adwordsCampaignID and a.date = c.date left join (SELECT a.date,a.adwordsCampaignID, sum(a.`totalEvents`) as `calls` FROM `ga_event_category_history_data` a where a.`eventCategory` like '%Call%' or `eventCategory` like '%Form%' GROUP BY a.date,a.adwordsCampaignID ) d on a.adwordsCampaignID = d.adwordsCampaignID and a.date = d.date) WHERE b.campaign_type = 4 and b.client_id = '$target_number' and date(c.date )>= now()-interval 12 month and date(a.date )>= now()-interval 12 month";
  ...
  if(mysql_num_rows($rs_year_graph_data)>0){  
                                while($ga_year_graph_data = mysql_fetch_assoc($rs_year_graph_data)){
                                  ...
                                  $event[] = $ga_year_graph_data['event'];
								   $calls[] = $ga_year_graph_data['calls'];
                                }
                            }
							
//							echo " Event by the year graph: "."</br>";
//							echo "IMPRESSION"." : "."CLICK"." : "."EVENT"." : "."CALLS"."</br>" ;
								  
							$i=0;
							foreach ($impression as $impression_value) {
//								echo    $impression_value." : ".$click[$i]." : ".$event[$i]." : ".$calls[$i]."</br>" ;
//								$i++;
							}						
...
<!--                                                <script type="text/javascript">
                        ...
                        <div class="clearB"></div>
							...
							<?php 
								if($click[0] == ''){ $click[0] = 0;}
								if($calls[0] == ''){ $calls[0] = 0;}
								if($event[0] == ''){ $event[0] = 0;}
								if($impression_value == ''){ $impression_value = 0;}
							
							?>
						<iframe id="chartfunnel" src="https://server.cbo.me/sem/final_funnel_trend.php?id=<?php echo $target_number; ?>&click=<?php echo $click[0]; ?>&impression=<?php echo $impression_value; ?>&event=<?php echo $event[0]; ?>&calls=<?php echo $calls[0]; ?>" ></iframe>                 
                       	<style>
                        	#chartfunnel {width: 95%; height: 500px;font-size: 11px; float:right; clear:both; border:none;}	
                        </style>
                        <div class="clearB"></div>
