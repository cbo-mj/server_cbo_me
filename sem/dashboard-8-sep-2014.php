...
if(isset($_POST["submit"]))
{
	...
	if(isset($_POST["type"]) and $_POST["type"]!="")
	{
	 ...
	 $sql =" select ga.date as date, ifnull(sum(ga.impressions),0) as impressions, ifnull(sum(ga.adClicks),0) as adClicks, sum(ga.CTR) as CTR, ifnull(sum(gaw.totalEvents),0) as totalEvents, ifnull(sum(ca.total_call),0) as total_call from ga_adword_campaign_data_30_days ga left join (select c.date, c.client_id, count(*) as total_call from call_log c where c.client_id = '$client_id' group by c.date, c.client_id) ca on
		       ga.date = date(ca.date) left join campaign_event_30_day gaw on 
               ga.date = gaw.date and ga.`adwordsCampaignID` = gaw.adwordsCampaignID
		where ga.account_id = '$account_id' and 	ga.profile_id = '$profile_id' and ga.`adwordsCampaignID`='$campaign_id'
		and  gaw.adwordsCampaignID='$campaign_id'  
		group by ga.date
		 order by ga.date     ";
		     ...
		     $sql =" select ga.date as date, ifnull(sum(ga.impressions),0) as impressions, ifnull(sum(ga.adClicks),0) as adClicks, sum(ga.CTR) as CTR, ifnull(sum(gaw.totalEvents),0) as totalEvents, ifnull(sum(ca.total_call),0) as total_call from ga_adword_campaign_data_history ga left join (select c.date, c.client_id, count(*) as total_call from call_log c where c.client_id = '$client_id' group by c.date, c.client_id) ca on
		       ga.date = date(ca.date) left join campaign_event_history gaw on 
               ga.date = gaw.date and ga.`adwordsCampaignID` = gaw.adwordsCampaignID
		where ga.account_id = '$account_id' and 	ga.profile_id = '$profile_id' and ga.`adwordsCampaignID`='$campaign_id'
		and  gaw.adwordsCampaignID='$campaign_id' $date_sql 
		group by ga.date
		 order by ga.date     ";
			...
			if(mysql_num_rows($rs_30_days)>0)
		{
		  ...
		  $sql = "select ga.date as date, ifnull(sum(ga.impressions),0) as impressions, ifnull(sum(ga.adClicks),0) as adClicks, sum(ga.CTR) as CTR, ifnull(sum(ga.totalEvents),0) as totalEvents, ifnull(sum(ca.total_call),0) as total_call  from ga_adword_event_data_history ga left join (select c.date, c.client_id, count(*) as total_call from call_log c where c.client_id = '$client_id' group by c.date, c.client_id) ca on ga.date = date(ca.date) where ga.account_id = '$account_id' and ga.profile_id = '$profile_id' $date_sql group by ga.date order by ga.date";
		...
		if(mysql_num_rows($rs_30_days)>0)
		{
			...
			$sum_call_click_ratio = $sum_call_click_ratio / mysql_num_rows($rs_30_days);
			 ...
			 $sql = "select ga.date as date, ifnull(sum(ga.impressions),0) as impressions, ifnull(sum(ga.adClicks),0) as adClicks, sum(ga.CTR) as CTR, ifnull(sum(ga.totalEvents),0) as totalEvents, ifnull(sum(ca.total_call),0) as total_call  from ga_adword_event_data_30_days ga left join (select c.date, c.client_id, count(*) as total_call from call_log c where c.client_id = '$client_id' group by c.date, c.client_id) ca on ga.date = date(ca.date) where ga.account_id = '$account_id' and ga.profile_id = '$profile_id'  group by ga.date order by ga.date" ;
	     	$rs_30_days = mysql_query($sql) or die ( mysql_error()  );
