    ...
    AmCharts.makeChart("SBHOTD", {
		...
		<?php 
			...
			foreach ($hour as $key => $hour_value){
					$arr_hourly_sessions[] = "{\"monthly\": '{$hour_value}', \"sessions\":'{$hour_event[$key]}'}" ;
			}
		...
		<?php 
			...
			if(empty($week_day)|| $week_day == 0 || $week_day == ""){
				...
				foreach ($week_day as $key => $week_day_value) {
					...
					$arr_weekly_sessions[] = "{\"monthly\": '{$week}', \"sessions\":'{$week_event[$key]}'}" ;
				}
			}
			...
			//foreach ($week_day as $key => $week_day_value) {
				//echo    $week_day_value." : ".$week_event[$key];
			//}
    ...
    AmCharts.makeChart("SBMOTY", {
		...
		<?php 
			...
			foreach ($month_name as $key => $month_name_value) {
				...
				$arr_yearly_sessions[] = "{\"monthly\": '{$month}', \"sessions\":'{$month_event[$key]}'}" ;
			}
