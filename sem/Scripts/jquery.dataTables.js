...
else if(typeof cellData==='function'){return cellData();}
...
else{var cells=row.anCells;var cell;if(cells){for(i=0,ien=cells.length;i<ien;i++){cell=cells[i];while(cell.childNodes){cell.removeChild(cell.firstChild);}
...
{var classes=oSettings.oClasses;var table=$(oSettings.nTable);var holding=$('<div/>').insertAfter(table);var features=oSettings.oFeatures;var insert=$('<div/>',{id:oSettings.sTableId+'_wrapper','class':classes.sWrapper+(oSettings.nTFoot?'':' '+classes.sNoFooter)});oSettings.nTableWrapper=insert[0];oSettings.nTableReinsertBefore=oSettings.nTable.nextSibling;oSettings.nHolding=holding[0];var aDom=oSettings.sDom.split('');var featureNode,cOption,nNewNode,cNext,sAttr,j;for(var i=0;i<aDom.length;i++)
