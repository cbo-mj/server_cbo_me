...
if(isset($_POST["submit"]))
{
	...
	if(isset($_POST["type"]) and $_POST["type"]!="")
	{
		...
		}else if($type=="date_range")
		{
		 ...
		 $sql =" select ga.date as date, sum(ga.impressions) as impressions,
		 sum(ga.adClicks) as adClicks, sum(ga.CTR) as CTR, sum(gaw.totalEvents) as totalEvents,
		 count(*) as total_call from ga_adword_campaign_data_30_days ga left join call_log c on
		       ga.date = date(c.date) left join campaign_event_30_day gaw on 
		...
		group by ga.date
		 ...
		 // and c.client_id = '$client_id' 
			...
			if(mysql_num_rows($rs_30_days)>0)
			{
				...
				while($ga_30_days_detail = mysql_fetch_assoc($rs_30_days))
				{
					...
					$call_click_ratio = number_format((($total_call/$adClicks)*100),2);
		 ...
		 {
			$sql = "select ga.date as date, sum(ga.impressions) as impressions,
		 sum(ga.adClicks) as adClicks, sum(ga.CTR) as CTR, sum(ga.totalEvents) as totalEvents,
		 count(*) as total_call from ga_adword_event_data_30_days ga left join call_log c on
		       ga.date = date(c.date)
		where ga.account_id = '$account_id' and 	ga.profile_id = '$profile_id' 
		and c.client_id = '$client_id'
		group by ga.date
		 order by ga.date";
	     	$rs_30_days = mysql_query($sql);
