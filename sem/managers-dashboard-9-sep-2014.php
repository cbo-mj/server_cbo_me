...
(SELECT DISTINCT `adwordsCampaignID` FROM `ga_adword_campaign_data_30_days` WHERE `account_id`='$firstAccountId' AND `profile_id`='$profile_id') union all SELECT `group_campaign_id` as campaign_id,`campaign_name` FROM group_campaign WHERE client_id = '$client_id'";
...
/*	    $sql =" select ga.date as date, ifnull(sum(ga.impressions),0) as impressions, ifnull(sum(ga.adClicks),0) as adClicks, sum(ga.CTR) as CTR, ifnull(sum(gaw.totalEvents),0) as totalEvents, ifnull(sum(ca.total_call),0) as total_call from ga_adword_campaign_data_30_days ga left join (select date(c.date) as date, c.client_id, count(*) as total_call from call_log c where c.client_id = '$client_id' group by date(c.date), c.client_id) ca on
		...
		where ga.account_id = '$account_id' and 	ga.profile_id = '$profile_id' and ga.`adwordsCampaignID` in($campaign_id)
		and  gaw.adwordsCampaignID in($campaign_id)
		group by ga.date
		 order by ga.date     ";*/
		...
		echo  $sql = "select gaw.date as date, ifnull(sum(gaw.impressions),0) as impressions, ifnull(sum(gaw.adClicks),0) as adClicks, sum(gaw.CTR) as CTR, ifnull(sum(gaw.totalEvents),0) as totalEvents, ifnull(sum(ca.total_call),0) as total_call from
