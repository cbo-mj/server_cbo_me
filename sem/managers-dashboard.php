...
/*	    $sql =" select ga.date as date, ifnull(sum(ga.impressions),0) as impressions, ifnull(sum(ga.adClicks),0) as adClicks, sum(ga.CTR) as CTR, ifnull(sum(gaw.totalEvents),0) as totalEvents, ifnull(sum(ca.total_call),0) as total_call from ga_adword_campaign_data_30_days ga left join (select date(c.date) as date, c.client_id, count(*) as total_call from call_log c where c.client_id = '$client_id' group by date(c.date), c.client_id) ca on
		...
		if(mysql_num_rows($rs_30_days)>0)
		{
		  ...
		  (select ga.date as date, ifnull(sum(ga.impressions),0) as impressions, ifnull(sum(ga.adClicks),0) as adClicks, sum(ga.CTR) as CTR, ifnull(sum(ga.totalEvents),0) as totalEvents  from ga_adword_event_data_30_days ga  and ga.account_id = '$account_id' and ga.profile_id = '$profile_id'  group by ga.date) gaw 
		  left join (select c.date, c.client_id, count(*) as total_call from call_log c where c.client_id = '$client_id' group by c.date, c.client_id) ca on gaw.date = date(ca.date) group by gaw.date order by gaw.date" ;
	     	$rs_30_days = mysql_query($sql) or die ( mysql_error()  );
