...
class transmitsmsAPIbase {
	...
	/**
	 * URL of proxy server if you use one. Serves as a flag to enable proxy connection.
	 * 
	 * @var string
	 */	
	public $proxyURL = "";
	...
	public function __construct($apiKey, $apiSecret = "")
	{
		...
		if ($validKey) {
			...
			echo 'The API key passed is not in a valid format - passed: ' . $apiKey;
			exit;
		}
	}
	...
	protected function call_method($method, $params = array())	
	{
		...
		if (function_exists("curl_init")) {
			...
			curl_setopt($ch, CURLOPT_FOLLOWLOCATION, false);//true
			...
			$result = curl_exec($ch);
			if (! $result) {
				$httpHost=isset($_SERVER['HTTP_HOST'])?$_SERVER['HTTP_HOST']:_DOMAIN_RESELLER;
				exit("CURL: Problem executing request <strong>{$method}</strong> on <strong>{$httpHost}</strong>, try changing above set options and re-requesting: " . curl_errno($ch) . " : " . curl_error($ch) . "<br />");		
			}
			curl_close($ch);		
			return $result;
		}
		...
		elseif (ini_get("allow_url_fopen")) {
			...
			if ($sock) {
				...
				$requestStr = "request=".urlencode($xml);				
				$headers = array(
...
class transmitsmsAPI extends transmitsmsAPIbase {
	...
	/**
	 * Get users contact lists recipients with last delivery status
	 *
	 * @param integer $id
	 * @param integer $offset
	 * @param integer $limit
	 * @return xml string
	 */
	public function getContactListRecipientsDelivery($id, $offset = 0, $limit = 10)
	{
		return $this->call_method("contact-lists.get-recipients-delivery", array("id" => $id, "offset" => $offset, "limit" => $limit));
	}
	 ...
	 * @return xml string
	 */
	public function optoutContactListRecipient($list_id, $mobileIntFormat)
	{		
		return $this->call_method("contact-lists.optout-recipient", array("list_id" => $list_id, "mobile" => $mobileIntFormat));
	}
	 ...
	 * @return xml string
	 */
	public function getMessageOptOuts($message_id, $offset = 0, $limit = 10)
	{
		return $this->call_method("messages.opt-outs", array("message_id" => $message_id, "offset" => $offset, "limit" => $limit));
	}
	 ...
	 * @param string $repliesToEmail - send replies to this message to this email
	 * @return xml string
	 */
	public function SMS($mobile, $message, $caller_id, $sendTime = null, $autoAddContactListID = null, $repliesToEmail='')
	{
		return $this->call_method("messages.single", array("mobile" => $mobile, "message" => $message, "caller_id" => $caller_id, "sendtime" => $sendTime, "contact_list" => $autoAddContactListID, 'replies_to_email'=>$repliesToEmail));
	}
	 ...
	 * @return xml string
	 */
	public function SMSMulti($mobile, $message, $caller_id, $sendTime = null, $autoAddContactListID = null, $repliesToEmail='')
	{
		return $this->call_method("messages.multiple", array("mobile" => $mobile, "message" => $message, "caller_id" => $caller_id, "sendtime" => $sendTime, "contact_list" => $autoAddContactListID, 'replies_to_email'=>$repliesToEmail));
	}
	 ...
	 * @return xml string
	 */
	public function addClient($name, $company_name, $email, $password, $phone, $reseller_pays = 'no', $reseller_markup = 0)
	{
		return $this->call_method("clients.add", array("name" => $name,"name_company" => $company_name, "email" => $email, "password" => $password, "phone" => $phone, "reseller_pays" => $reseller_pays, "reseller_markup" => $reseller_markup));
	}
	 ...
	 * @return xml string
	 */
	public function getTransactionsForClients($offset = 0, $limit = 10, $dateStart = null, $dateEnd = null, $type='all')
	{
		return $this->call_method("transactions.get-clients", array("offset" => $offset, "limit" => $limit, "dateStart" => $dateStart, "dateEnd" => $dateEnd, "type"=>$type));		
	}
	...
	/**
	 * Associate email with Email SMS, verify successful addition with 
	 * response 'EMAIL_OK'
	 *
	 * @param string $email
	 * @param integer $messages_limit
	 * @param bool $international
	 * @param bool $send_dlr
	 * @param integer $longcode
	 * @return xml string
	 */
	public function addEmail($email, $messages_limit=1, $international=false, $send_dlr=false, $longcode=0)
	{
		return $this->call_method("emails.add", array("email" => $email, "messages_limit" => $messages_limit, "international" => $international, "send_dlr"=>$send_dlr, "longcode"=>$longcode));
	}
	
	/**
	 * Get list of active user keywords
	 * 
	 * @param int $offset
	 * @param int $limit
	 * @param int $number
	...
	public function getKeywords($offset=0, $limit=10, $number='')
	{
		return $this->call_method('keywords.get', array('offset'=>$offset, 'limit'=>$limit, 'number'=>$number));
	}
	
	/**
	 * Returns responses for specified keyword
	 * 
	 * @param int $id
	 * @param int $offset
	 * @param int $limit
	 */
	public function getKeywordResponses($id, $offset=0, $limit=10)
	{
		return $this->call_method('keywords.responses', array('id'=>$id, 'offset'=>$offset, 'limit'=>$limit));
	} 
	
	/**
	 * Returns responses from the responder, optionally filtered by longcode or specific keyword
	 * 
	 * @param int $mobile
	 * @param int $longcode
	 * @param string $keyword
	 */
	public function getKeywordResponder($mobile, $longcode=null, $keyword=null)
	{
		return $this->call_method('keywords.responder', array('mobile'=>$mobile, 'longcode'=>$longcode, 'keyword'=>$keyword));
	}
	
	/**
	 * Returns list of actual SMS sent, during the time period
	 *  
	 * @param int $offset
	 * @param int $limit
	 * @param date $dateStart (Y-m-d H:i:s)
	 ...
	 */
	public function getMessageStats($offset=0, $limit=100, $dateStart=null, $dateEnd=null)
	{
		return $this->call_method('message-stats.get', array("offset" => $offset, "limit" => $limit, "dateStart" => $dateStart, "dateEnd" => $dateEnd)); 
	}
	/**
	 * Returns the quantity of messages sent from provided number, during the time period
	 * 
	 * @param int $number
	 * @param date $dateStart
	 * @param date $dateEnd
	 */
	public function getNumberStatsSMS($number, $dateStart=null, $dateEnd=null)
	{
		return $this->call_method('number-stats.total-sms', array("number" => $number, "dateStart" => $dateStart, "dateEnd" => $dateEnd)); 
	}
	
	/**
	 * Returns the quantity of SMS sent from provided number, during the time period
	 * 
	 * @param int $number
	 * @param date $dateStart
	 * @param date $dateEnd
	 */	
	public function getNumberStatsMessages($number, $dateStart=null, $dateEnd=null)
	{
		return $this->call_method('number-stats.total-messages', array("number" => $number, "dateStart" => $dateStart, "dateEnd" => $dateEnd)); 
	}	
	
	/**
	 * Get an array of your leased numbers
	 * 
	 */
	public function getNumbers()
	{
		return $this->call_method('numbers.get', array());
	}
	
	/**
	 * Get an array of numbers available for lease
	 * 
	 */
	public function getAvailableNumbers()
	{
		return $this->call_method('numbers.get-available', array());
	}

	/**
	 * Lease a number
	 * 
	 * @param int $number if not provided, a random available number will be leased
	 */
	public function leaseNumber($number=0)
	{
		return $this->call_method('numbers.lease', array('number'=>$number));
	}
	 ...
	 * @param int $number 
	 * @param string $keyword
	 * @param int $list_id - optional, list where new responders will be added
	 * @param string $name - optional, keyword name
	 * @param string $reply_message - optional, auto-reply SMS for new responder 
	 * @param string $reply_message_dupe - optional, auto-reply SMS for existing responder
	 * @param string $pass_to_url - optional, callback URL to post reply
	 * @param string $pass_to_email - optional, list of emails (comma separated) to forward reply 
	 * @param string $pass_to_sms - optional, list of numbers (comma separated) to forward reply as SMS
	 * @param boolean $activate - activate keyword after creation 
	 */
	public function addKeyword($number, $keyword, $list_id=0, $name='', $reply_message='', $reply_message_dupe='', $pass_to_url='', $pass_to_email='', $pass_to_sms='', $activate=true)
	{
		return $this->call_method('keywords.add', array('number'=>$number, 'keyword'=>$keyword, 'list_id'=>$list_id, 'name'=>$name, 'reply_message'=>$reply_message,  'reply_message_dupe'=>$reply_message_dupe, 'pass_to_url'=>$pass_to_url, 'pass_to_email'=>$pass_to_email, 'pass_to_sms'=>$pass_to_sms, 'activate'=>$activate));
	}
	 ...
	 * 
	 * @param int $id
	 * @param array $params - values to be updated. can have following keys - list_id, name, reply_message, reply_message_dupe, pass_to_url, pass_to_email, pass_to_sms. only add keys for fields you want to update 
	 */
	
	public function editKeyword($id, $params)
	{
		$params['id']=$id;
		return $this->call_method('keywords.edit', $params);
	}
	 ...
	 * 
	 * @param int $id
	 * @param string $status - can be open, paused, closed, deleted
	 */
	public function setKeywordStatus($id, $status)
	{
		return $this->call_method('keywords.set-status', array('id'=>$id, 'status'=>$status));
	}
	
	/**
	 * get message delivery results
	 * 
	 * @param int $id  -message ID
	 * @param int $offset
	 * @param int $limit
	 */
	public function getMessageDelivery($id, $offset=0, $limit=10)
	{
		return $this->call_method('messages.delivery', array('id'=>$id, 'offset'=>$offset, 'limit'=>$limit));
	}
	...
	{
		return $this->call_method('profile.get-balance', array());
	}
}
