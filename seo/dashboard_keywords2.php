...
FROM authority_lab_ranks_30_days a WHERE a.group_id = $al_group_id and a.domain_id = $al_domain_id
  ...
  $sql_rank_client = "SELECT distinct b.name as name, CASE a.`google_rank` WHEN '' THEN 0 ELSE a.`google_rank` END google_rank ,  CASE a.`google_change` WHEN '' THEN 0 ELSE a.`google_change` END g_change, a.google_total_results as volume, CASE  a.local_pack WHEN '' THEN 0 ELSE a.`local_pack` END map,CASE  a.images WHEN '' THEN 0 ELSE a.`images` END images, CASE  a.blog WHEN '' THEN 0 ELSE a.`blog` END blog,CASE  a.news WHEN '' THEN 0 ELSE a.`news` END news,CASE  a.shopping WHEN '' THEN 0 ELSE a.`shopping` END shopping,CASE  a.micro_format WHEN '' THEN 0 ELSE a.`micro_format` END micro_format, CASE  a.video WHEN '' THEN 0 ELSE a.`video` END video 
FROM `authority_lab_ranks_30_days` a
INNER JOIN authority_lab_keywords b ON a.`keywords_id` = b.`keywords_id`
and a.`group_id` = b.`group_id` and 
...
a.group_id = c.group_id $date_sql
            ...
            <?php foreach ($name as $key => $name_value): ?>
                <tr class="odd">
                    ...
                    <td class="call_data"><?php //echo $map[$key].' '.$images[$key].' '.$video[$key]; ?>
                    	<?php 
							if($map[$key] != 0){ echo " <span title='Local results found.'><img src='keywords/icons/maps.png'></span> "; }
							if($images[$key] != 0){ echo " <span title='Image results found.'><img src='keywords/icons/image_icon.png'></span> "; }
							if($video[$key] != 0){ echo " <span title='Video results found.'><img src='keywords/icons/video.png'></span> "; }
							if($blog[$key] != 0){ echo " <span title='Blogs results found.'><img src='keywords/icons/blog.png'></span> ";}
							if($news[$key] != 0){ echo " <span title='News results found.'><img src='keywords/icons/news.png'></span> ";}
							if($shopping[$key] != 0){ echo " <span title='Shopping results found.'><img src='keywords/icons/shopping.png'></span> ";}
							if($micro_format[$key] != 0){ echo " <span title='Rich snippets results found.'><img src='keywords/icons/micro_format.png'></span> ";}
						?>                  
                    </td>
                </tr>
			<?php endforeach; ?>                
