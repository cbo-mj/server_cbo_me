...
if(isset($_POST["submit"]))
{
	...
	if(isset($_POST["type"]) and $_POST["type"]!="")
	{
		...
		if($type=="today")
		{
			...
			$date_sql = "and date(a.`rank_date) = '$today_date' ";
		}else if($type=="yesterday")
		{
			$yesterday_date = date('Y-m-d ', strtotime(" -1 day "));
			$date_sql = "and date(a.`rank_date`) = '$yesterday_date' ";
		}else if($type=="last_week")
		{
			$date_sql = " and a.`rank_date` = curdate() - INTERVAL DAYOFWEEK(curdate())+6 DAY";
		}else if($type=="last_month")
		{	
			$date_sql = " and date(a.`rank_date` ) = now()-interval 1 month	";
		}else if($type=="last_3_months")
		{
			$date_sql = "  and date(a.`rank_date` ) = now()-interval 3 month		";
		}else if($type=="last_6_months")
		{
			$date_sql = "  and date(a.`rank_date` ) = now()-interval 6 month		";
		}else if($type=="last_year")
		{
			$date_sql = " and  a.`rank_date` = DATE_SUB(NOW(),INTERVAL 1 YEAR)		";
		}else if($type=="date_range")
		{
			$from_date = $_POST["from"];
			$to_date = $_POST["to"];
			$to_date = date("Y-m-d",strtotime($to_date . "+1 day"));
			$date_sql = " and  a.`rank_date` between '$from_date' AND '$to_date'		";
		}
...
if(mysql_num_rows($rs_rank_client) > 0 )
	{
		...
		while($client_rank_detail = mysql_fetch_assoc($rs_rank_client))
		{
		 ...
		 if($date_sql!="")
		 {
		  ...
		  $sql_rank_client_d = "SELECT distinct b.name as name, CASE a.`google_rank` WHEN '' THEN 0 ELSE a.`google_rank` END google_rank
FROM `authority_lab_ranks_30_days` a
INNER JOIN authority_lab_keywords b ON a.`keywords_id` = b.`keywords_id`
WHERE a.group_id = $al_group_id and a.domain_id = $al_domain_id and b.name='$key_name' $date_sql" ; 
		  ...
		  $sql_rank_client_d = "SELECT distinct b.name as name, CASE a.`google_rank` WHEN '' THEN 0 ELSE a.`google_rank` END google_rank
FROM `authority_lab_ranks_365_days` a
INNER JOIN authority_lab_keywords b ON a.`keywords_id` = b.`keywords_id`
WHERE a.group_id = $al_group_id and a.domain_id = $al_domain_id and b.name='$key_name' $date_sql" ; 
	...
	//	echo $sql_rank_client_d;
		
	//		echo '<br/>';
		 
		 $rs_new_rank = mysql_query($sql_rank_client_d) or die ( $sql_rank_client_d . mysql_error());
		 
		 if(mysql_num_rows( $rs_new_rank)>0)
		 {
			$new_rank_detail = mysql_fetch_assoc( $rs_new_rank );
			$google_rank_new = $new_rank_detail["google_rank"];
		 }
		 
		 
		 }
        
		//echo $key_name.$client_rank_detail['google_rank'];;
		//echo '<br/>';
		...
		if($google_rank_new=="")
		{
			...
			$g_change[] = $client_rank_detail['google_rank'] - $google_rank_new;
		}
		
		//echo $key_name. $google_rank_new;
