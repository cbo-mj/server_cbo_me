...
if(isset($_POST["submit"]))
{
   ...
   $sql_hour = "SELECT `hour`,`sessions` as session FROM `ga_todays_data_session` where account_id = '$account_id' and profile_id = '$profile_id' order by hour asc"; 
	$sql_week = "SELECT DAYNAME(`date`) as day,`ga_sessions` as session FROM `ga_data_30_days` WHERE weekofyear(`date`) = weekofyear(now())-1 and account_id = '$account_id' and profile_id = '$profile_id' ";
