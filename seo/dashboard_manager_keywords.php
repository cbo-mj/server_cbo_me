        ...
        <table id="call_tracking_record" class="display"  width="100%" style="border-left:1px solid #dbdbda; border-bottom:1px solid #dbdbda;">
     		<thead>
                <tr class="nobg">
                	<th></th>
                    <th width="250"></th>
                    <th colspan="3" class="center border"><img src="keywords/icons/google.png"></th>
                    <th colspan="3" class="center top"><img src="keywords/icons/yahoo.png"></th>
                    <th colspan="3" class="center border"><img src="keywords/icons/bing.png"></th>
                    <th width="150"></th>                                  
                </tr>
                <tr class="heading">
                	<th>#</th>
            ...
            <tbody>
            <?php $name = array_unique($name); ?>
            <?php foreach ($name as $key => $name_value): ?>
                <tr class="odd">
                	<td><?php echo $key; ?></td>
                    <td class="call_data"><?php echo $name_value; ?></td>
                    <td  class="call_data">
						<?php 
							if($g_i_rank[$key] != 0 || $g_i_rank[$key] != '-0' ){ echo $g_i_rank[$key]; }else{ echo '-';}
						?>
                    </td>
                    <td class="call_data">
						<?php 
							if($g_rank[$key] != 0 || $g_rank[$key] != '-0'){ echo $g_rank[$key]; }else{ echo '-';}
						?>	
                    </td>
                    <td class="call_data">
						<?php 
							if($g_change[$key] != 0 || $g_change[$key] != '-0'){ echo $g_change[$key]; }else{ echo '-';}
						?>
                    </td>
                    <td class="call_data">
						<?php 
							if($y_i_rank[$key] != 0 || $y_i_rank[$key] != '-0'){ echo $y_i_rank[$key]; }else{ echo '-';}
						?>                    
					</td>
                    <td class="call_data">
						<?php 
							if($y_rank[$key] != 0 || $y_rank[$key] != '-0'){ echo $y_rank[$key]; }else{ echo '-';}
						?>                      
					</td>
                    <td class="call_data">
						<?php 
							if($y_change[$key] != 0 || $y_change[$key] != '-0'){ echo $y_change[$key]; }else{ echo '-';}
						?> 	                    
					</td>
                    <td class="call_data">
						<?php 
							if($b_i_rank[$key] != 0 || $b_i_rank[$key] != '-0'){ echo $b_i_rank[$key]; }else{ echo '-';}
						?>                     
                    </td>
                    <td class="call_data">
						<?php 
							if($b_rank[$key] != 0 || $b_rank[$key] != '-0'){ echo $b_rank[$key]; }else{ echo '-';}
						?>                    
					</td>
                    <td class="call_data">
						<?php 
							if($b_change[$key] != 0 || $b_change[$key] != '-0'){ echo $b_change[$key]; }else{ echo '-';}
						?>                     
                    </td>
                    <td><?php //echo $map[$key].' '.$images[$key].' '.$video[$key]; ?>
                    	<?php 
