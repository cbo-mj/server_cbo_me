...
FROM `authority_lab_ranks_30_days` a inner join (select `keywords_id`,max(`rank_date`) as j_date from `authority_lab_ranks_30_days` where group_id = $al_group_id and domain_id = $al_domain_id group by `keywords_id`) b on a.`rank_date` = b.j_date and 
a.`keywords_id` = b.`keywords_id`
...
inner join (select `keywords_id`,max(`rank_date`) as j_date from `authority_lab_ranks_30_days` where group_id = $al_group_id and domain_id = $al_domain_id group by `keywords_id`) b on a.`rank_date` = b.j_date and 
a.`keywords_id` = b.`keywords_id` 
WHERE a.group_id = $al_group_id and a.domain_id = $al_domain_id " ; 
...
a.group_id = c.group_id $date_sql inner join (select `keywords_id`,max(`rank_date`) as j_date from `authority_lab_ranks_30_days` where group_id = $al_group_id and domain_id = $al_domain_id group by `keywords_id`) b on a.`rank_date` = b.j_date and 
a.`keywords_id` = b.`keywords_id`
WHERE a.group_id = $al_group_id and a.domain_id = $al_domain_id   
