...
if($_GET["search"]){
	...
	if(isset($_GET["type"]) and $_GET["type"]!=""){
		...
		$sql_pie_call = "SELECT count(*) as `call` FROM `call_log` WHERE ".$where." `client_id`='$target_number' and `status_code` in(0,1,3) $sql_campaign_id   union all SELECT count(*) as `call` FROM `call_log` WHERE ".$where."  `client_id`='$target_number' and `status_code` = 1  $sql_campaign_id  union all SELECT count(*) as `call` FROM `call_log` WHERE ".$where."  `client_id`='$target_number' and `status_code` = 0  $sql_campaign_id " ;
		$sql_pie_avg = "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(`duration`))/COUNT(*)) AS `avg_time` FROM `call_log` WHERE ".$where."  `client_id`='$target_number' and `status_code` in(0,1,3)  $sql_campaign_id " ;
		...
		$sql_pie_call = "SELECT count(*) as `call` FROM `call_log` WHERE `client_id`='$target_number' and `status_code` in(0,1,3) and `date` >= NOW() - INTERVAL 1 MONTH union all SELECT count(*) as `call` FROM `call_log` WHERE `client_id`='$target_number' and `status_code` = 1 and `date` >= NOW() - INTERVAL 1 MONTH union all SELECT count(*) as `call` FROM `call_log` WHERE `client_id`='$target_number' and `status_code` = 0 and `date` >= NOW() - INTERVAL 1 MONTH " ;
		$sql_pie_avg = "SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(`duration`))/COUNT(*)) AS `avg_time` FROM `call_log` WHERE `client_id`='$target_number' and `status_code` in(0,1,3) and `date` >= NOW() - INTERVAL 1 MONTH " ;
