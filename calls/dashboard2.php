...
<?php 
	$rs_call_log2 = mysql_query($nstr) or die ( mysql_error());
	$rs_call_log = mysql_query($sql) or die ( mysql_error() );
	
	$a = array();	
	
	while($row2 = mysql_fetch_assoc($rs_call_log2))
	{
	$date_d = strtotime($row2['date']);
	$date_d = date("Y-m-d", $date_d);		
	//rsort($date_d);
	//foreach($row as $d) {
		if($row2['status_name'] == 'Abandoned'){ $missed = 1; $answered = 0; }else{ $missed = 0; $answered = 1;}
		$total = $missed + $answered;
		$a[] = "{\"date\": '{$date_d}',\"Missed\":'{$missed}',\"Total\":'{$total}',\"Answered\":'{$answered}'}";
	//}
...
?>        
		<div class="clearB"></div>
		<?php include('callsLineChart.php'); ?>
        <div class="linegrap-wrap"><div id="newlinechartdiv" style="width: 100%; height: 300px; background-color: #FFFFFF;" ></div></div>
