...
$sql_keyword_list = "select * from authority_lab_keywords where group_id = 5596 AND 
						domain_id = 190191 ";
...
$sql_get_start_date = "SELECT MAX(rank_date) AS start_date FROM  `authority_lab_ranks_365_days` WHERE group_id = '$group_id' AND domain_id = '$domain_id' and keywords_id = $keywords_id" ;
 ...
 $rs_date = mysql_query($sql_get_start_date) or die ( mysql_error() ) ;
		   ...
		   $sql_check_new_table_keyword_date = "SELECT * FROM  `authority_lab_ranks_365_days` WHERE group_id = '$group_id' AND domain_id = '$domain_id' and keywords_id = '$keywords_id' and rank_date= '$startDate'  " ;
				...
				while(strtotime($startDate) <= strtotime($endDate))
				{
					...
					if(!empty($ranks_list))
					{
						...
						for($r=0;$r<$sizer;$r++)
						{
							...
							//$keywords_id = $ranks_list['ranks'][$r]['keyword_id'];
							...
							$sql_check = "
							SELECT * FROM authority_lab_ranks_365_days					
...
if(mysql_num_rows($rs_keyword_list)>0)
{
	while($keyword_detail = mysql_fetch_assoc($rs_keyword_list))
	{
			...
			$group_id = $keyword_detail["group_id"];
 				...
 				$sql_get_start_date = "SELECT MAX(rank_date ) AS start_date FROM  `authority_lab_ranks_history` where group_id = '$group_id' AND domain_id = '$domain_id' and keywords_id = $keywords_id" ;
			...
			while(strtotime($startDate) <= strtotime($endDate))
			{
				...
				if(!empty($ranks_list))
				{
					...
					for($r=0;$r<$sizer;$r++)
					{
						...
						$sql_check = "
						SELECT * FROM authority_lab_ranks_history					
						WHERE group_id = '$group_id' and domain_id = '$domain_id_r' and keywords_id ='$keywords_id' and rank_id='$rank_id' and rank_date = '$rank_date' ";
...
echo 'cron job run succesfully for 365 days rank date authority lab';
