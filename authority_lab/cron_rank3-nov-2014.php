...
$sql_domain_list = "select distinct group_id , domain_id  from authority_lab_keywords where domain_id = 194075 and group_id = 5596";
...
$sql_keyword_list = "select * from authority_lab_keywords where domain_id = 194075 and group_id = 5596";
...
if(mysql_num_rows($rs_keyword_list)>0)
{
	while($keyword_detail = mysql_fetch_assoc($rs_keyword_list))
	{
			...
			$keyword_name = $keyword_detail["name"];

$sql_get_start_date = "SELECT MAX(rank_date) AS start_date FROM  `authority_lab_ranks_30_days` WHERE group_id = '$group_id' AND domain_id = '$domain_id' and keywords_id = $keywords_id " ;
