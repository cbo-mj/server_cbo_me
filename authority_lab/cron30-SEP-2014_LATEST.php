...
if(!empty($group_list))
{
	...
	for($i=0;$i<$sizeg;$i++)
	{
		...
		$sql_delete_30_days = " delete from authority_lab_ranks_30_days where 
                        rank_date < date_add(NOW(),INTERVAL -31 DAY)";  
      
  	 mysql_query($sql_delete_30_days) or die ( mysql_error() ) ;

 		$sql_get_start_date = "SELECT MAX(rank_date ) AS start_date FROM  `authority_lab_ranks_30_days`" ;
      ...
      // echo $startDate ; echo "</br>";
       $endDate =  date('Y-m-d', strtotime('today - 1 days')); echo "</br>";
			...
			if(!empty($ranks_list))
			{
				...
				for($r=0;$r<$sizer;$r++)
				{
					...
					if( mysql_num_rows($rs_company_log) == 0 )
					{
						...
						if( $rank_date =="" and $google_change =="" and
		...
		$sql_delete_30_days = " delete from authority_lab_ranks_365_days where 
                        rank_date < date_add(NOW(),INTERVAL -365 DAY)";  
      
  	 mysql_query($sql_delete_30_days) or die ( mysql_error() ) ;

 		$sql_get_start_date = "SELECT MAX(rank_date ) AS start_date FROM  `authority_lab_ranks_365_days`" ;
