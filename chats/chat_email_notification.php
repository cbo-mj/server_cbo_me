...
include("include/common_function.php");
	 ...
	 $sql_chat_detail = "select c.*,en.`email` as n_email from chat_lead_info c inner join chat_company_info co on c.companyKey = co.companyKey inner join client cl on SUBSTRING_INDEX(SUBSTRING_INDEX(co.`googleAnalyticsAccount`, '-', 2),'-',-1) = cl.account_id inner join email_notification en on cl.client_id = en.client_id inner join services s on s.service_id= en.service_id where date(c.`createdOn`) = '$today_date' and c.email_send = '0' and s.service_name ='chat' and en.`email`<>''";  
		...
		//	echo $message; 
			...
			$shane_email ="shane.mcgeorge@cbo.me,arjay@cbo.me,arjay00lumbres@gmail.com" ;
