...
<form method="post">
	...
	if(isset($account_ids) and !empty($account_ids))
	{
		$account_id_not_in = implode(",",$account_ids);
		$sql_delete_account = 
			"
			delete 
			FROM  `ga_account` 
			where 
			account_id NOT IN ($account_id_not_in) and email = '$email'
			";
			
			mysql_query($sql_delete_account) or die ( mysql_error() ) ;
			
			
			$sql_delete_ga_property = 
			"
			delete 
			FROM  `ga_property` 
			where 
			...
			mysql_query($sql_delete_ga_property) or die ( mysql_error() ) ;
