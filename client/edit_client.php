...
if(isset($_POST["submit"]))
{
	...
	if($_POST["notification_email"]!="")
	{
		if(!filter_var($_POST["notification_email"], FILTER_VALIDATE_EMAIL))
		{
			...
			//$error["notification_email"] = "Please enter valid notification email !";
		}
	}
	...
	if(empty($error))
	{
			...
			$Zendesk_company_ID = $_POST["Zendesk_company_ID"];
			$Support_purchased = $_POST["Support_purchased"];
...
<form method="post">
   ...
   <h3 style="color:black;"> <?php if(isset($msg)){echo $msg;} ?></h3>
	...
	$sql_check_account = 
						"
						SELECT * 
						FROM  `ga_account` where account_id  order by account_name
						";
          ...
          <tr>
            ...
            <?php
			 ...
			 $sql_check_account = 
						"
						SELECT * 
						FROM  `authority_lab_group` order by name
						";;
			?>
...
?>
      ...
      <tr>
        	<td valign="top" class="tbl_label">Notification Email</td>
            <td><input type="text" name="notification_email" id="notification_email" value="<?php if($_POST["notification_email"]){ echo $_POST["notification_email"];}else{  echo $client_detail["notification_email"];} ?>" />
            <br/>
            <span style="color:blue;"><?php if(isset($error["notification_email"])){ echo $error["notification_email"]; }?></span>
            </td>        
        </tr>
        
        
         <tr>
        	<td valign="top" class="tbl_label">Zendesk company ID</td>
            <td><input type="text" name="Zendesk_company_ID" id="Zendesk_company_ID" value="<?php if($_POST["Zendesk_company_ID"]){ echo $_POST["Zendesk_company_ID"];}else{  echo $client_detail["Zendesk_company_ID"];} ?>" />
            <br/>
            <span style="color:blue;"><?php if(isset($error["Zendesk_company_ID"])){ echo $error["Zendesk_company_ID"]; }?></span>
            </td>        
        </tr>
        
        
         <tr>
        	<td valign="top" class="tbl_label">Support purchased</td>
            <td><input type="text" name="Support_purchased" id="Support_purchased" value="<?php if($_POST["Support_purchased"]){ echo $_POST["Support_purchased"];}else{  echo $client_detail["Support_purchased"];} ?>" />
            <br/>
            <span style="color:blue;"><?php if(isset($error["Support_purchased"])){ echo $error["Support_purchased"]; }?></span>
            </td>        
        </tr>
