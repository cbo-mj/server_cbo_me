...
$sql_query = "SELECT * FROM email_notification WHERE notification_id = '$notification_id' ";

$result = mysql_query($sql_query);

$email_notication_detail = mysql_fetch_assoc($result);
...
if(isset($_POST["submit"]))
{
	...
	if(empty($error))
	{
		$sql = "select * from email_notification where service_id = '$service_id' and client_id = '$client_id' and notification_id != '$notification_id' ";
	...
	if(empty($error))
	{
		...
		$updated_dtm  = date("Y-m-d H:i:s");
		 ...
		 $sql_update = "update email_notification set
						email = '$email',
						client_id = '$client_id',
		...
		$msg = "Email notification has been updated successfully";
...
<form method="post" action="">
	<p><strong>Edit Email Notification</strong></p>
	...
	if(mysql_num_rows($rs_service)>0)
	{
	 while($service_detail = mysql_fetch_assoc($rs_service))
	 {
		 ...
		 {	
			...
			if($service_detail["service_id"]==$email_notication_detail['service_id'])
			 {	
				$selected_acc = 'selected="selected"';
	...
	if(mysql_num_rows($rs_client)>0)
	{
	 while($client_detail = mysql_fetch_assoc($rs_client))
	 {
		 ...
		 {	
			...
			if($client_detail["client_id"]==$email_notication_detail['client_id'])
			 {	
				$selected_acc = 'selected="selected"';
    ...
    <br/>
       ...
       <tr>
            ...
            <td>
             ...
             <textarea style="height:100px; width:300px;" type="text" name="email" id="email"><?php if($_POST["email"]){ echo $_POST["email"];}else{ echo $email_notication_detail['email'];} ?></textarea>
        ...
        <tr>
            <td align="center" colspan="3"><input type="submit" name="submit" id="submit" value="Update" /></td>        
        </tr>
    </table>
</form>
</body>
