        ...
        <div id="container-body">
            <section class="main-section grid_8">
				...
				$sql = "
						...
						select a.date as dt,medium,`adwordsCampaignID`,`eventCategory`,sum(a.ga_sessions) as total_s,sum(b.te) as total_e from ((select `date`,`ga_sessions` from ga_data_365_days where `account_id`='$account_id' and `profile_id`='$profile_id') a left join (SELECT `date` as date1,`medium`,`adwordsCampaignID`,`eventCategory`, sum(`totalEvents`) as te FROM `ga_event_category_history_data` where `account_id`='$account_id' and `profile_id`='$profile_id' group by `date`,`medium`,`adwordsCampaignID`,`eventCategory`) b on a.date = b.date1) group by dt,b.medium,`adwordsCampaignID`,`eventCategory` order by dt desc
				...
				";
                            ...
                            if(mysql_num_rows($rs_30_days)>0){
                                while($ga_30_days_detail = mysql_fetch_assoc($rs_30_days)){
									$dt[] = $ga_30_days_detail['dt'];
									$medium[] = $ga_30_days_detail['medium'];
									$adwordsCampaignID[] = $ga_30_days_detail['adwordsCampaignID'];
									$eventCategory[] = $ga_30_days_detail['eventCategory'];
									$total_s[] = $ga_30_days_detail['total_s'];
									$total_e[] = $ga_30_days_detail['total_e'];
                            }
                        }
				echo "Date : Medium : Adwords Campaign ID : Event Category : Total Visist : Total Event</br>";
				$i=0;
				foreach($dt as $dt_value){
					
		echo $dt_value." : ".$medium[$i]." : ".$adwordsCampaignID[$i]." : ".$eventCategory[$i]." : ".$total_s[$i]." : ".$total_e[$i]."</br>";
